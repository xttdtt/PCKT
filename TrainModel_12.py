...
final_joint_embed = np.load(os.path.join(modelfolder, "final_joint_embed.npz"), allow_pickle=True)
final_true_corr = np.load(os.path.join(modelfolder, "final_true_corr.npz"), allow_pickle=True)
final_joint_embed = final_joint_embed["final_joint_embed"]
final_true_corr = final_true_corr["final_true_corr"]
...
tf_embed_target = tf.placeholder(tf.float32, [None, None], name='tf_data_embed')  # [256,2]
tf_corr_target = tf.placeholder(tf.float32, [None], name='tf_data_corr')  # 256
tf_keep_rate = tf.placeholder(tf.float32, None, name="tf_keep_rate")  # 0.5

pred_w = tf.get_variable('pred_w', [1, 1 + 1], initializer=tf.truncated_normal_initializer(stddev=0.1))
...
logfile = os.path.join(modelfolder, "trainModellog.txt")
f = open(logfile, "wb+")
f.truncate()
...
logging.info('problem number %d, skill number %d, student number %d' % (num_pro, num_skill, num_stu))
logging.info("data_num {0},train_data_num {1},test_data_num {2}".format(data_num, len(train_joint_data), len(test_joint_data)))
msg = "split_rate=" + str(split_rate) + "  epochs=" + str(epochs) + "  bs=" + str(bs) + "  lr=" + str(lr) + "  keep_rate=" + str(keep_rate) + "  early_stop=" + str(early_stop)
logging.info(msg)
...
with tf.Session() as sess:
    ...
    for i in range(epochs):
        ...
        records = 'Epoch %d/%d, train loss:%.4f, test auc:%.4f, test acc:%.4f,epoch time:%f' % \
                  (i + 1, epochs, train_loss, test_auc, test_acc, epochendtime - epochstarttime)
        logging.info(records)
        if best_auc < test_auc or best_acc < test_acc:
            best_auc = test_auc
            best_acc = test_acc
