...
# Find the weights of the skill embedding and multiply them together
skill_embedding_matrix = tf.multiply(skill_embedding_matrix, tf.nn.softmax(skill_embedding_matrix, axis=0))
# Problem-related skills embedding matrix
...
# Optimization of students' mastery of skills
...
logfile = os.path.join(modelfolder, "trainEmbeddinglog.txt")
f = open(logfile, "wb+")
f.truncate()
...
msg = "epochs=" + str(epochs) + "  bs=" + str(bs) + "  embed_dim=" + str(embed_dim) + "  lr=" + str(lr) + " early_stop=" + str(early_stop)
logging.info(msg)
    ...
    for i in range(epochs):
        ...
        if i >= early_stop:
            if all(x <= y for x, y in zip(Loss[tmp:tmp + early_stop], Loss[tmp + 1:tmp + 1 + early_stop])):
                ...
                break
            tmp += 1
    logging.info("best_loss=%f in epoch %d" % (best_loss, best_epoch))
    saver.save(best_sess, os.path.join(modelfolder, 'trainEmbedding_%d.ckpt' % (best_epoch)))
    logging.info('finish training...')

    final_pro_embed = tf.convert_to_tensor(pro_embedding_matrix)
    final_pro_embed = final_pro_embed.eval()
    final_pro_embed = np.array(final_pro_embed)
